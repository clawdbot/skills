...
const { markdownEscape } = require('./utils');

class Plan2MealCommands {
  constructor(convexClient, githubOAuth, config) {
    this.convex = convexClient;
    this.oauth = githubOAuth;
    this.config = config;
  }
  ...
  async authenticate(state) {
    const authUrl = this.oauth.getAuthUrl(state);
    return {
      text: `🔐 Please authenticate with GitHub:\n\n[Click here to authorize](${authUrl})`,
      requiresAuth: false
    };
  }
   ...
   * Add recipe from URL
   */
  async addRecipe(githubToken, url) {
    ...
    const recipe = await this.convex.createRecipe({
      ...
      localization: metadata.localization || { language: 'en' }
    });
   ...
   * Search recipes
   */
  async searchRecipes(githubToken, term) {
   ...
   * Show recipe details
   */
  async showRecipe(githubToken, id) {
    ...
    return {
      text: this.formatRecipeFull(recipe)
    };
  }
   ...
   * Delete recipe
   */
  async deleteRecipe(githubToken, id) {
   ...
   * Create grocery list
   */
  async createList(githubToken, name) {
   ...
   */
  async addRecipeToList(githubToken, listId, recipeId) {
  ...
  formatRecipePreview(metadata) {
    ...
    if (metadata.ingredients?.length) {
      text += `🥘 **Ingredients** (${metadata.servings || '?'} servings)\n`;
      metadata.ingredients.slice(0, 5).forEach(ing => {
        text += `• ${markdownEscape(ing)}\n`;
      });
      if (metadata.ingredients.length > 5) {
        text += `  └─ ...and ${metadata.ingredients.length - 5} more\n`;
      }
    }

    if (metadata.steps?.length) {
      text += `\n🔪 **Steps**\n`;
      metadata.steps.slice(0, 3).forEach((step, i) => {
        text += `${i + 1}. ${markdownEscape(step.slice(0, 100))}${step.length > 100 ? '...' : ''}\n`;
      });
    }
  ...
  formatRecipeFull(recipe) {
    ...
    text += this.formatTime(recipe.prepTime, recipe.cookTime) + '\n';
